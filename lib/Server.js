...
function startSling(instanceName) {
    ...
    if (!isSlingInstalled(instanceName)) {
      ...
      const jarPath = path.join(getDownloadDirForServer(instanceName), 'org.apache.sling.kickstart-0.0.12.jar')
      const farPath = path.join(getDownloadDirForServer(instanceName), 'feature-oak_tar_fds.json')
