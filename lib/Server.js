...
function listServers() {
  ...
  if (fs.existsSync(serverConfig)) {
      ...
      console.log('instance'.padEnd(12), 'running'.padEnd(8), 'port'.padEnd(6), 'runmodes'.padEnd(24), 'path')
      settings.forEach( (server) => {
        let runmodes = server.runmodes ? server.runmodes : ''  
        console.log(server.name.padEnd(12), isSlingRunning(server.name).toString().padEnd(8), server.port.toString().padEnd(6), runmodes.padEnd(24), server.path)
      })
  } else {
