...
function startSling(instanceName) {
    ...
    if (!isSlingInstalled(instanceName)) {
      ...
      const jarPath = path.join('..', config.download.dir, 'org.apache.sling.kickstart-0.0.12.jar')
      const farPath = path.join('..', config.download.dir, 'feature-oak_tar_fds.json')
...
function getPidFile(instanceName) {
  let installPath = getPathForServer(instanceName);
  return installPath ? path.join(installPath, config.sling.pid) : undefined
}
